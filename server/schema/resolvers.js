...
const resolvers = {
    Query: {
        ...
        receivedFriendRequests: async (_, __, {authUser}) => {
            if (!authUser) throw new AuthenticationError("Not Authenticated!!");
            return await User.find({_id: {$in: authUser.receivedFriendRequests}}).exec();
        },
        sentFriendRequests: async (_, __, {authUser}) => {
            if (!authUser) throw new AuthenticationError("Not Authenticated!!");
            return await User.find({_id: {$in: authUser.sentFriendRequests}}).exec();
        },
    ...
    Mutation: {
        register: async (_, {username, password, firstName, lastName, bio, avatar}) => {
        ...
        createNewPost: async (_, {image, caption}, {authUser}) => {
            ...
            await User.findByIdAndUpdate(authUser.id, {$push: {posts: newPost.id}, $inc: {postCount: 1}}).exec();
        ...
        deletePost: async (_, {postId}, {authUser}) => {
            ...
            const authorIdOfPost = await Post.findById(postId).select('author').exec();
            ...
            await User.findByIdAndUpdate(authUser.id, {$pull: {posts: postId, $dec: {postCount: 1}}}).exec();
        ...
        createNewComment: async (_, {postId, info}, {authUser}) => {
            ...
            const postCommented = await Post.findById(postId).select("id").exec();
            ...
            await Post.findByIdAndUpdate(postId, {$push: {comments: newComment.id}, $inc: {commentCount: 1}}).exec();
        ...
        deleteComment: async (_, {postId, commentId}, {authUser}) => {
            ...
            const authorIdOfComment = await Comment.findById(commentId).select('author').exec();
            ...
            await Post.findByIdAndUpdate(postId, {$pull: {comments: commentId}, $dec: {commentCount: 1}}).exec();
            return true;
        },
        likePost: async (_, {userId, postId}, {authUser}) => {
            ...
            const postToLike = await Post.findById(postId).exec();
            if (!postToLike) throw new Error("Post not found")
            console.log(`Post liked by ${authUser.username}`);
            await Post.findByIdAndUpdate(postId, {$addToSet: {likes: userId}, $inc: {likeCount: 1}}).exec();
        ...
        unlikePost: async (_, {postId}, {authUser}) => {
            ...
            const postToUnlike = await Post.findById(postId).exec();
            if (!postToUnlike) throw new Error("Post not found");
            console.log("Post unliked");
            await Post.findByIdAndUpdate(postId, {$pull: {likes: authUser.id}, $dec: {likeCount: 1}}).exec();
            return true;
        },
        sendFriendRequest: async (_, {toUserId}, {authUser}) => {
            ...
            await User.findByIdAndUpdate(authUser.id, {$addToSet: {sentFriendRequests: toUserId}}).exec();
            pubsub.publish(FRIEND_REQUEST_RECEIVED, {friendRequestReceived: {fromUserId: authUser.id}});
        ...
        cancelFriendRequest: async (_, {toUserId}, {authUser}) => {
            ...
            const toUser = await User.findByIdAndUpdate(toUserId, {$pull: {receivedFriendRequests: authUser.id}}, {new: true}).select("id").exec();
            console.log(toUser);
            if (!toUser) {
                throw new Error('User not found');
            }
            await User.findByIdAndUpdate(authUser.id, {$pull: {sentFriendRequests: toUserId}}).exec();
            return true;
        },
        respondToFriendRequest: async (_, {toUserId, response}, {authUser}) => {
            ...
            const toUser = await User.findById(toUserId).select("id").exec();
            console.log(toUser);
            if (!toUser) throw new UserInputError("User Not Found");
            await User.findByIdAndUpdate(authUser.id, {$pull: {receivedFriendRequests: toUserId}}).exec();
            if (response) {
                await User.findByIdAndUpdate(authUser.id, {$addToSet: {friends: toUserId}}).exec();
                await User.findByIdAndUpdate(toUserId, {$addToSet: {friends: authUser.id}}).exec();
                pubsub.publish(FRIEND_REQUEST_ACCEPTED, {friendRequestAccepted: {acceptorId: toUserId}});
            }
            return true;
        },
        removeFriend: async (_, {friendId}, {authUser}) => {
            ...
            const friendUser = await User.findByIdAndUpdate(friendId, {$pull: {friends: authUser.id}}, {new: true}).select("id").exec();
            console.log(friendUser);
            if (!friendUser) throw new UserInputError("User Not Found");
            await User.findByIdAndUpdate(authUser.id, {$pull: {friends: friendId}}).exec();
            return true;
        },
        createNewChat: async (_, {withUserId}, {authUser}) => {
            ...
            const isChat = await Chat.countDocuments({participants: {$all: [authUser.id, withUserId]}}).exec();
            ...
            await User.findByIdAndUpdate(authUser.id, {$push: {chats: newChat.id}}).exec();
            await User.findByIdAndUpdate(withUserId, {$push: {chats: newChat.id}}).exec();
            return newChat;
        },
        sendMessage: async (_, {chatId, receiverId, text}, {authUser}) => {
            ...
            try {
                ...
                pubsub.publish(MESSAGE_RECEIVED, {messageReceived: {chatId, message: newMessage}})
        ...
        deleteMessage: async (_, {messageId, chatId, receiverId}, {authUser}) => {
            if (!authUser) {
                throw new AuthenticationError("Not Authenticated");
            }
            const messageToDelete = await Message.countDocuments({_id: messageId}).exec();
            if (!messageToDelete) throw new Error("message not found");
            await Message.findByIdAndDelete(messageId).exec();
            await Chat.findByIdAndUpdate(chatId, {$pull: {messages: messageId}}).exec();
            return true;
        }
    },
    Subscription: {
        ...
        friendRequestAccepted: {
            subscribe: withFilter(
                ...
                (payload, variables) => payload.friendRequestAccepted.acceptorId === variables.acceptorId
            ),
            resolve: async (payload) => {
                return await User.findById(payload.friendRequestAccepted.acceptorId.exec());
            }
        },
        friendRequestReceived: {
            subscribe: withFilter(
                () => pubsub.asyncIterator('FRIEND_REQUEST_RECEIVED'),
                (payload, variables) => payload.friendRequestReceived.fromUserId === variables.fromUserId
            ),
            resolve: async (payload) => {
                return await User.findById(payload.friendRequestReceived.fromUserId).exec();
            }
        },
        postCommented: {
            subscribe: withFilter(
                () => pubsub.asyncIterator('POST_COMMENTED'),
                (payload, variables) => payload.postCommented.postId === variables.postId
            ),
            resolve: async (payload) => {
        ...
        messageReceived: {
            subscribe: withFilter(
                ...
                (payload, variables) => payload.messageReceived.chatId === variables.chatId
            ),
            resolve: async (payload) => {
    ...
    User: {
        ...
        receivedFriendRequests: async (parent) => {
            return await User.find({_id: {$in: parent.receivedFriendRequests}}).exec();
        },
        sentFriendRequests: async (parent) => {
            return await User.find({_id: {$in: parent.sentFriendRequests}}).exec();
        },
        chats: async (parent) => {
